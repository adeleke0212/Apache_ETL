...
def clean_merchants_data():
    ...
    df['isActive'] = df['isActive'].apply(
        lambda x: 'true' if x == 'yes' or x == 'true' else 'false')
...
def agg_customer_order_rejection():
    ...
    all_customers = read_csv_from_lake(
        clean_s3_uri.format(bucket_name, 'customers'))
    new_df = all_customers[all_customers['numberOfOrderRejections'] > 3][[
        'firstName', 'lastName', 'numberOfOrderRejections']]
    write_csv_to_lake(agg_s3_uri.format(
        bucket_name, 'customers', 'customer_order_rejection'), new_df)
...
def agg_customers_per_state():
    ...
    write_csv_to_lake(agg_s3_uri.format(
        bucket_name, 'customers', 'customers_per_state'), new_df)
...
def agg_total_orders_per_month():
    ...
    new_df = all_orders[['_id', 'year', 'month']
                        ].groupby(by=['year', 'month']).count()
    new_df.rename({'_id': 'total_orders'}, inplace=True)
    write_csv_to_lake(agg_s3_uri.format(
        bucket_name, 'orders', 'total_orders_per_month'), new_df)
...
def agg_orders_list():
    ...
    all_customers = read_csv_from_lake(
        clean_s3_uri.format(bucket_name, 'customers'))
    all_orders = read_csv_from_lake(clean_s3_uri.format(bucket_name, 'orders'))
    all_merchants = read_csv_from_lake(
        clean_s3_uri.format(bucket_name, 'merchants'))
    ...
    new_df = new_df[['_id_x', 'meal', 'name', 'orderedBy', 'orderDate', 'promoCode']]\
        .merge(all_customers[['_id', 'firstName', 'lastName', 'state', 'city', 'registration_date']], how='left', left_on='orderedBy', right_on='_id')[['_id_x', 'meal', 'name', 'orderedBy', 'orderDate', 'promoCode', 'firstName', 'lastName', 'state', 'city', 'registration_date']]
    print('==================================\n', new_df)
    new_df.rename({'name': 'merchant', '_id_x': 'orderID'}, inplace=True)
    write_csv_to_lake(agg_s3_uri.format(
        bucket_name, 'orders', 'orders_list'), new_df)
