...
with DAG(
    ...
    # ========== Extraction
    create__bucket = PythonOperator(
        task_id='create__bucket', python_callable=create_bucket)

    extract__customers = PythonOperator(
        task_id='extract__customers', python_callable=extract_customers_to_lake)
    extract__orders = PythonOperator(
        task_id='extract__orders', python_callable=extract_orders_to_lake)
    extract__merchants = PythonOperator(
        task_id='extract__merchants', python_callable=extract_merchants_to_lake)

    # ========== Cleaning

    clean__customers_data = PythonOperator(
        task_id='clean__customers_data', python_callable=clean_customers_data)
    clean__orders_data = PythonOperator(
        task_id='clean__orders_data', python_callable=clean_orders_data)
    clean__merchants_data = PythonOperator(
        task_id='clean__merchants_data', python_callable=clean_merchants_data)

    agg__execution = DummyOperator(task_id='agg__execution')

    # ========== Aggregation

    agg__customer_order_rejection = PythonOperator(
        task_id='agg__customer_order_rejection', python_callable=agg_customer_order_rejection)
    agg__customers_per_state = PythonOperator(
        task_id='agg__customers_per_state', python_callable=agg_customers_per_state)
    agg__orders_list = PythonOperator(
        task_id='agg__orders_list', python_callable=agg_orders_list)
    agg__total_orders_per_month = PythonOperator(
        task_id='agg__total_orders_per_month', python_callable=agg_total_orders_per_month)
